...
import { Youtube, Instagram, Linkedin, MessageCircle, Plus, Trash2, RefreshCw, CheckCircle2, AlertCircle, ExternalLink, X } from 'lucide-react'
...
export default function PlatformsPage() {
  ...
  const [tgForm, setTgForm] = useState({ channelId: '', channelUsername: '', channelName: '', botToken: '', loading: false })

  useEffect(() => {
    const connected = params.get('connected')
    const error = params.get('error')
    if (connected) { toast.success(`✅ ${connected} muvaffaqiyatli ulandi!`); window.history.replaceState({}, '', '/platforms') }
  ...
  const addTelegram = async () => {
    ...
    try {
      await socialAPI.addTelegramChannel({
        ...
        bot_token: tgForm.botToken || undefined,
      })
      toast.success('✅ Telegram kanal qo\'shildi!')
      setModal(false)
      setTgForm({ channelId: '', channelUsername: '', channelName: '', botToken: '', loading: false })
      fetchAll()
    } catch (err: any) {
          ...
          {PLATFORMS.map((p, i) => {
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  {acc ? (
                    <span className="flex items-center gap-1.5 text-xs text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-full border border-emerald-500/20">
                      <CheckCircle2 size={11} /> Ulangan
                    </span>
                  ) : (
                    <span className="text-xs text-slate-500 bg-slate-800/60 px-2.5 py-1 rounded-full border border-slate-700/40">
                      Ulanmagan
                    </span>
                  )}
                </div>
                ...
                {acc && (
                  <div className="mt-3 p-3 rounded-xl" style={{ background: `${p.color}08`, border: `1px solid ${p.color}20` }}>
                    <p className="text-slate-300 text-sm font-medium truncate">{acc.account_name || 'Connected'}</p>
                    {acc.expires_at && (
                      <p className="text-slate-500 text-xs mt-0.5">Tugash: {new Date(acc.expires_at).toLocaleDateString()}</p>
                    )}
                  </div>
                )}
                ...
                <div className="flex gap-2 mt-4">
                  {acc ? (
                    <>
                      ...
                      >
                        <RefreshCw size={13} className={connecting === p.key ? 'animate-spin' : ''} />
                        Qayta ulash
                      </button>
                      <button
                        onClick={() => disconnect(acc.id, p.label)}
                    ...
                    >
                      {connecting === p.key ? (
                        <><RefreshCw size={13} className="animate-spin" /> Yo'naltirilmoqda...</>
                      ) : (
                        <><ExternalLink size={13} /> {p.label} ulash</>
                      )}
                    </motion.button>
                  )}
                </div>
        ...
        >
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">OAuth shart emas — Bot API orqali</p>
              </div>
            </div>
            <button
          ...
          {tgChannels.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-slate-600 text-xs mt-1">@tendermarketing01bot ni kanalingizga admin qiling</p>
            </div>
          ) : (
            <div className="space-y-3">
              {tgChannels.map(ch => (
                <div key={ch.id} className="flex items-center justify-between p-4 rounded-xl" style={{ background: 'rgba(14,165,233,0.06)', border: '1px solid rgba(14,165,233,0.15)' }}>
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">Faol</span>
                    <button onClick={() => { socialAPI.deleteTelegramChannel(ch.id); setTgChannels(t => t.filter(x => x.id !== ch.id)); toast.success('Kanal o\'chirildi') }} className="p-2 rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-500/10">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
        ...
        {modal && (
          ...
          >
            ...
            >
              ...
              <div className="p-4 rounded-xl mb-5" style={{ background: 'rgba(14,165,233,0.08)', border: '1px solid rgba(14,165,233,0.2)' }}>
                <p className="text-sky-300 text-sm font-medium mb-2">📋 Bosqichlar:</p>
                <ol className="text-slate-400 text-xs space-y-1.5 list-decimal ml-4">
                  <li><strong className="text-sky-400">@tendermarketing01bot</strong> ni kanalingizga qo'shing</li>
                  <li>Uni <strong className="text-white">Admin</strong> qiling (Post yozish huquqi)</li>
                  <li>Kanal ID ni oling: <strong className="text-sky-400">@username_to_id_bot</strong> ga kanaldan forward qiling</li>
                  <li>ID quyiga kiriting (odatda <code className="text-sky-400">-100</code> bilan boshlanadi)</li>
                </ol>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setTgForm(f => ({ ...f, channelId: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setTgForm(f => ({ ...f, channelUsername: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    value={tgForm.channelName}
                    onChange={e => setTgForm(f => ({ ...f, channelName: e.target.value }))}
              ...
              <div className="flex gap-3 mt-5">
                <button onClick={() => setModal(false)} className="flex-1 py-2.5 rounded-xl text-slate-400 text-sm border border-slate-700/50 hover:bg-white/5">
                  Bekor
                </button>
                <motion.button
                  whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  onClick={addTelegram}
                  disabled={tgForm.loading || !tgForm.channelId.trim()}
